...
# If you have error about the bellow error like "./setup.sh: line 70: [: too many arguments", it's not an error
if [[ ! `grep "$VIRTUAL_HOST" "$HOSTS_PATH"` ]]; then
    echo 'Adding DNS entries';
    sudo sh -c 'echo "${NEWLINE}127.0.0.1 '${VIRTUAL_HOST}'" >> /etc/hosts';
fi;
...
sleep 3;
docker exec ${APP_NAME}_platform_1 bash -c "cd /var/www/html && php artisan migrate && php artisan passport:install";
